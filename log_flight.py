...
def setup_logger(
    ...
    flogger.enableConfig("current")
    flogger.enableConfig("bat_volt")
...
def setup_optitrack(optitrack):
    ...
    return ot_position, ot_attitude, fan_position
...
def receive_rigidbody_frame(id, position, rotation):
    ...
    global flogger, ot_position, ot_attitude, fan_position

    # Check ID
    if id == fan_id:
        fan_position = util.ot2control(position)
    if id == ot_id:
        # Register position
...
def build_trajectory(trajectories, space, fan_position):
    ...
    # Build trajectory
    # Takeoff
    fan_offset_x = fan_position[0] + 1
    fan_offset_y = fan_position[1] + 2
    setpoints = takeoff(home["x"] + fan_offset_x, home["y"] + fan_offset_y, altitude, 0.0)
    for trajectory in trajectories:
        ...
        if trajectory == "nothing":
            ...
            setpoints += hover(home["x"] + fan_offset_x, fan_position[1], altitude)
    ...
    # Add landing
    setpoints += landing(home["x"] + fan_offset_x, home["y"] + fan_offset_y, altitude, 0.0)
...
if __name__ == "__main__":
    ...
    fan_id = 1
    ot_position, ot_attitude, fan_position = setup_optitrack(args["optitrack"])
    # Wait for fix
    if ot_position is not None:
        while (ot_position == 0).any():
            ...
            time.sleep(1)
        print(f'fan position: {fan_position}')
    ...
    # Build trajectory
    setpoints = build_trajectory(args["trajectory"], args["space"], fan_position)
